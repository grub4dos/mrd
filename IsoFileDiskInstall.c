...
EFI_STATUS
	...
	)
	{
		...
		Status=FindPartitionInIsoFile(FileDiskFileHandle,&pridata[1].StartAddr,&pridata[1].Size,&pridata[2].StartAddr,&pridata[2].Size);
		if(EFI_ERROR (Status)) {
			Print(L"No bootable partition on the iso image!\n");
		...
		for(i=0;i<3;i++){
			...
			pridata[i].Media.IoAlign 		  		= 2;				//不添加这个参数不能被diskio识别
